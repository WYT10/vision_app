...
static bool append_test_csv(const std::string& path,
    ...
    if (!exists) {
        out << "timestamp,device,width_req,height_req,fps_req,fourcc,width_actual,height_actual,frames,elapsed_sec,fps_avg,fps_min,fps_max,frame_ms_avg,frame_ms_min,frame_ms_max,discarded_grabs,read_failures,detect_ms_avg,warp_ms_avg,total_ms_avg,detect_ms_max,warp_ms_max,total_ms_max,detector_fps_avg,tag_id,warp_width,warp_height\n";
    }
...
static bool write_latest_report_md(const std::string& path,
    ...
    if (stage) {
        ...
        out << "- Detector FPS avg: " << stage->detector_fps_avg << "\n\n";
    }
    ...
    if (lock && lock->valid) {
        ...
        out << "- Tag ID: `" << lock->id << "`\n";
        out << "- Warp size: `" << lock->warp_size.width << 'x' << lock->warp_size.height << "`\n\n";
    }
